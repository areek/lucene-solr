...
public class TestContextQuery extends LuceneTestCase {
  ...
  @Test
  public void testContextQueryWithBoostFirstScoreMode() throws Exception {
    Analyzer analyzer = new MockAnalyzer(random());
    RandomIndexWriter iw = new RandomIndexWriter(random(), dir, iwcWithSuggestField(analyzer, "suggest_field"));
    Document document = new Document();
    ...
    int maxWeight = 4;
    assertSuggestions(suggest,
        new Entry("suggestion4", "type4", 1 + (maxWeight * 4)),
        new Entry("suggestion3", "type3", 2 + (maxWeight * 3)),
        new Entry("suggestion2", "type2", 3 + (maxWeight * 2)),
        new Entry("suggestion1", "type1", 4 + (maxWeight)));
  ...
  @Test
  public void testContextQueryWithIgnoreBoostScoreMode() throws Exception {
    Analyzer analyzer = new MockAnalyzer(random());
    RandomIndexWriter iw = new RandomIndexWriter(random(), dir, iwcWithSuggestField(analyzer, "suggest_field"));
    Document document = new Document();
  ...
  @Test
  public void testContextQueryScoring() throws Exception {
    Analyzer analyzer = new MockAnalyzer(random());
    RandomIndexWriter iw = new RandomIndexWriter(random(), dir, iwcWithSuggestField(analyzer, "suggest_field"));
    ...
    for (int i = 1; i <= numContexts; i++) {
      CharSequence context = "context_" + i;
      for (int j = 1; j <= numSuggestions; j++) {
        String suggestion = "suggestion_" + j;
        Document document = new Document();
        document.add(new ContextSuggestField("suggest_field", Collections.singletonList(context), suggestion, j));
        iw.addDocument(document);
        expectedEntries.add(new Entry(suggestion, context.toString(), i * j));
      }
      if (rarely()) {
        iw.commit();
      }
    ...
    ArrayUtil.introSort(expectedResults, new Comparator<Entry>() {
      ...
      public int compare(Entry o1, Entry o2) {
        ...
        if (cmp != 0) {
          ...
          return o2.output.compareTo(o1.output);
        }
      }
    });
