...
import org.apache.lucene.search.suggest.document.CompletionQuery.ScoreMode;
...
public class CompletionScorer extends BulkScorer {
  ...
  protected CompletionScorer(final CompletionWeight weight, final NRTSuggester suggester,
                             final ScoreMode scoreMode, final LeafReader reader, final Bits acceptDocs,
                             final boolean filtered, final Automaton automaton,
                             final long minWeight, final long maxWeight) throws IOException {
    this.weight = weight;
   ...
   * index time weight.
   * see {@link CompletionQuery#setScoreMode(ScoreMode)}
   * to change score formula
   */
  public final float score(float weight, float boost) {
    return scoreMode.score(weight, boost, minWeight, maxWeight);
  }
}
