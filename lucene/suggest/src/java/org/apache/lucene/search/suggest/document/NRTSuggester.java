...
import static org.apache.lucene.search.suggest.document.NRTSuggester.WeightProcessor.decode;
 ...
 * </p>
 * <p>
 * See {@link #lookup(LeafReader, Automaton, int, DocIdSet, LeafCollector)} for more implementation
 * details.
 * <p>
 ...
 * @lucene.experimental
 */
public class NRTSuggester implements Accountable {
  ...
  /**
   * Maximum queue depth for {@link Util.TopNSearcher}
   */
   ...
   * @lucene.experimental
   */
  public void lookup(final LeafReader reader, final Automaton automaton, final int num, final DocIdSet filter, final LeafCollector collector) {
    assert num > 0 : "num must be > 0, found num=" + num;
    assert reader != null : "reader must not be null";
    assert automaton != null : "automaton must not be null";
    assert collector != null : "collector must not be null";
    ...
    final TopSuggestDocsCollector suggestDocsCollector;
    if (collector instanceof TopSuggestDocsCollector) {
      suggestDocsCollector = (TopSuggestDocsCollector) collector;
    } else {
      suggestDocsCollector = null;
    }
    ...
    try {
      ...
      Util.TopNSearcher<Pair<Long, BytesRef>> searcher = new Util.TopNSearcher<Pair<Long, BytesRef>>(fst, num, queueSize, getComparator()) {
        ...
        protected boolean acceptResult(IntsRef input, Pair<Long, BytesRef> output) {
          ...
          try {
            if (suggestDocsCollector != null) {
              suggestDocsCollector.collect(docID, spare.toCharsRef(), decode(output.output1));
            } else {
  ...
  private int getMaxTopNSearcherQueueSize(int num, LeafReader reader, boolean filterEnabled) {
    ...
    int maxQueueSize = num * maxAnalyzedPathsPerOutput;
    // liveDocRatio can be at most 1.0 (if no docs were deleted)
    assert liveDocsRatio <= 1.0d;
    if (filterEnabled) {
      liveDocsRatio = (liveDocsRatio > 0.5) ? 0.5 : liveDocsRatio;
    }
    return Math.min(MAX_TOP_N_QUEUE_SIZE, (int) (maxQueueSize / liveDocsRatio));
  }
  ...
  /**
   * Helper to encode/decode weight output
   */
  static final class WeightProcessor {
