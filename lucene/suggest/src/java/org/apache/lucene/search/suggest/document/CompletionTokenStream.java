 ...
 * The accepted strings enumeration from the automaton are available through the {@link TermToBytesRefAttribute} attribute
 * The token stream uses a {@link org.apache.lucene.analysis.tokenattributes.PayloadAttribute} to store
 * a completion's payload (see {@link #setPayload(org.apache.lucene.util.BytesRef)})
 *
 * @lucene.experimental
 */
final class CompletionTokenStream extends TokenStream {
   ...
   * in {@link #setPayload(org.apache.lucene.util.BytesRef)}
   */
  public int sepLabel() {
    return sepLabel;
  }
